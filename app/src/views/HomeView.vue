<script setup lang="ts">
  import router from "@/router"
  import homepage from "../assets/homepage.jpg"
  import { Button } from "primevue"
  import Testimonial from "@/components/Homepage/Testimonial.vue"
  import { getSpaceColor } from "@/utils/getColor"

  type Average = {
    icon: string
    label: string
  }
  const averages: Average[] = [
    {
      icon: "pi-chart-bar",
      label: "Suivi des dépenses par catégorie",
    },
    {
      icon: "pi-users",
      label: "Partage entre membres d’un groupe",
    },
    {
      icon: "pi-planner",
      label: "Vue mensuelle / annuelle",
    },
  ]
</script>

<template>
  <div class="p-10">
    <section class="flex gap-10 items-center">
      <div class="flex flex-col gap-6">
        <p class="text-6xl font-bold">Monéa</p>
        <p class="text-2xl font-light text-gray-700">Gérez vos budgets en toute simplicité.</p>
        <p>
          Une application intuitive pour suivre vos dépenses, planifier l’avenir et garder le
          contrôle de vos finances personnelles ou en groupe.
        </p>
        <div class="flex gap-3">
          <Button
            label="Se connecter"
            @click="router.push({ name: 'login' })"
            :class="[getSpaceColor({ color: 'pink' })]"
          />
          <Button
            label="Créer un compte gratuitement"
            @click="router.push({ name: 'register' })"
            :class="[getSpaceColor({ color: 'pink' })]"
          />
        </div>
      </div>
      <img class="w-1/2 h-full rounded" :src="homepage" />
    </section>
    <section class="flex justify-center gap-5 my-20">
      <div class="w-48 flex flex-col items-center" v-for="(average, i) in averages" :key="i">
        <p class="text-6xl text-center mb-3 pi pi-chart-bar text-gray-700" :class="average.icon" />
        <p class="text-center">{{ average.label }}</p>
      </div>
    </section>
    <Testimonial />
  </div>
</template>

<style></style>
