...
<template>
  <BaseSection label="Catégorie sélectionné" v-if="model.length">
    <div class="flex gap-1.5 flex-wrap">
      <ColoredLabelComponent v-for="(category, i) in model" :category="category" add :key="i">
        <template #add>
  ...
  <BaseSection label="Ajouter une catégorie">
    <div class="flex gap-1.5 flex-wrap">
      <ColoredLabelComponent v-for="(category, i) in categories" :category="category" add :key="i">
        <template #add>
