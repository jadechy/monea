import FormExpense from "@/components/Display/FormExpense.vue"
...
describe("ExpenseForm.vue", () => {
  ...
  it("affiche le formulaire et soumet une création de dépense", async () => {
    const wrapper = mount(FormExpense, {
      props: { space_id: 123 },
      global: {
        stubs: ["SubHeader", "WrapperInput", "FormInput", "Recurring", "BaseSection"],
      },
    })
    ...
    const values = Object.fromEntries(
      Object.entries(formComponent.states).map(([key, fieldState]) => [key, fieldState.value]),
    )
  ...
  it("affiche bouton modifier et supprime la dépense", async () => {
    ...
    await wrapper.find('button:contains("Supprimer la dépense")').trigger("click")
