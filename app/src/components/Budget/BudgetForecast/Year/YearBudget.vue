  ...
  <BaseSection label="Budget restant mois par mois">
    <template #header>
      <Select
        ...
        :options="categories?.filter((category) => category.label !== 'default')"
        optionLabel="label"
        placeholder="Choisir une catégorie"
        class="ml-auto md:w-56"
        :class="[`bg-${selectedCategory?.color}-50`, `hover:bg-${selectedCategory?.color}-100`]"
    ...
    />
    <section class="grid gap-6 grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 mt-1" v-if="group">
